...
from marcovica import minimize_risk, maximize_return, getMaxReturn, getMinRisk, getStocksData, getMinReturn, getMaxRisk, getPortfolioHistory
import datetime
import pandas as pd
import numpy as np
...
def show_main_page():
    ...
    if ('stocks' not in st.session_state or 'fromDate' not in st.session_state or 'toDate' not in st.session_state) or (st.session_state.fromDate != fromDate or st.session_state.toDate != toDate):
        ...
        min_return = float(getMinReturn(st.session_state.stocks)) * 100 + 0.01
    ...
    if clicked:
            if profile == 'Максимизация доходности':
                ...
                history = getPortfolioHistory(10000, weights, st.session_state.stocks)
                weights = pd.DataFrame(weights, columns=weights.keys(), index=["Доля в портфеле (%)"])
                weights = weights.select_dtypes(exclude=['object', 'datetime']) * 100
                st.table(weights)
                #print(weights)
                labels = weights.columns.values
                sizes = weights.iloc[0]
                ...
                print('idrn', history)
                st.line_chart(history, x='date', y='value')
                ...
                weights = pd.DataFrame(weights, columns=weights.keys(), index=["Доля в портфеле (%)"])
                weights = weights.select_dtypes(exclude=['object', 'datetime']) * 100
                weights.sort_values(by=['Доля в портфеле (%)'], inplace=True)
                st.table(weights)
                labels = weights.columns.values
                sizes = weights.iloc[0]
